...
int main( void ){
    ...
    CfgPtrBtns = XGpio_LookupConfig(XGPIO_AXI_BTN_BASEADDR);
    if(CfgPtr == NULL){
        xil_printf("Error looking up the buttons gpio config\r\n");
        return XST_FAILURE;
    }
    status = XGpio_CfgInitialize(&Gpio_btns, CfgPtrBtns, XGPIO_AXI_BTN_BASEADDR);
    if(status != XST_SUCCESS){
    ...
    XGpio_SetDataDirection(&Gpio_btns, LED_CHANNEL, 0xFF); // input
...
static void checkGPIOs_Task( void *pvParameters )
{
    ...
    while (1) {
        ...
        btnState = XGpio_DiscreteRead(&Gpio_btns, LED_CHANNEL);
        if(btnState > 0){ // any button pressed
